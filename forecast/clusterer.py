...
class Clusterer:
    ...
    def __init__(self, dataframe, n_samples=10000, rho=0.8):
        ...
        self._dbgname = {v: k for k, v in
                         dict(enumerate(self._get_queries())).items()}
    ...
    def _build_neighbors(centers, timestamps, n_neighbors):
        ...
        if len(samples) == 0:
            ...
            normalized_samples = sklearn.preprocessing.normalize(samples,
                                                                 copy=False)
    ...
    def _cluster(self):
        ...
        for gap in trange(self.n_gaps):
            next_time = current_time + datetime.timedelta(
                seconds=self.cluster_gap)
            # Up to last 10 seconds.
            start_time = max(self.min_time,
                             next_time - datetime.timedelta(seconds=10))
            timestamps = Clusterer._sample_timestamps(self.n, start_time,
                                                      next_time,
                                                      self.n_samples)
            ...
            for template in last_assignment:
                ...
                if old_cluster is not None:
                    counts = Clusterer._query_df_range(self._df, template,
            ...
            neighbors = Clusterer._build_neighbors(centers, timestamps,
                                                   n_neighbors=1)
            ...
            for template in self._get_queries():
                ...
                if old_cluster is not None:
                    ...
                    still_belongs = Clusterer._similarity(
                        Clusterer._query_df(self._df, template,
                                            timestamps).values,
                        Clusterer._query_series(centers[old_cluster],
                                                timestamps).values
                    ) > rho
                    ...
                    centers[old_cluster] = centers[old_cluster].sub(
                        Clusterer._query_df_range(self._df, template,
                ...
                if neighbors == None:
                    for cluster in centers.keys():
                        if Clusterer._similarity(
                                self._query_df(self._df, template,
                                               timestamps).values,
                                self._query_series(centers[cluster],
                                                   timestamps).values
                        ) > rho:
                            new_cluster = cluster
                            break
                    ...
                    neighbor = \
                        neighbors.kneighbors(data, return_distance=False)[0][0]
                    clusters = sorted(centers.keys())
                    similarity = Clusterer._similarity(
                        data, centers[clusters[neighbor]].values)
                    if similarity > rho:
                ...
                if new_cluster is not None:
                    description = 'joined' \
                        if assignment[template] is None else 'reassigned to'
                    ...
                    centers[new_cluster] = centers[new_cluster].add(
                        self._query_df_range(self._df, template, start_time,
                                             next_time),
                        fill_value=0)
                ...
                centers[next_cluster] = self._query_df_range(self._df,
                                                             template,
                                                             start_time,
                                                             next_time)
                ...
                # print(f'WARNING: cluster {next_cluster} has no items. Does the following query appear within the lookback window: {self._dbgname[template]}')
            ...
            if len(clusters) > 1:
                ...
                for i, cluster in enumerate(clusters):
                    merge_cluster = None
                    data = \
                        Clusterer._query_series(centers[cluster], timestamps)[
                            'count'].values.reshape(1, -1)
                    data = sklearn.preprocessing.normalize(data)
                    neighbor = neighbors.kneighbors(data,
                    ...
                    is_similar = self._similarity(
                        self._query_series(centers[cluster],
                                           timestamps).values,
                        self._query_series(centers[clusters[neighbor]],
                                           timestamps).values) > rho
                    if cluster != clusters[neighbor] and is_similar:
                        merge_cluster = clusters[neighbor]
                    if merge_cluster != None:
                        centers[merge_cluster] = centers[merge_cluster].add(
                            centers[cluster], fill_value=0)
            ...
            current_time = next_time
            empty_clusters = set(
                cluster for cluster, df in centers.items()
                if df.shape[0] == 0)
    ...
    def _generate(self, clusterer):
        ...
        for current_ts, assignment in tqdm(clusterer.assignments):
            ...
            for template, cluster in assignment.items():
                ...
                query = query_cum.query(
                    "`query_template` == @template and `log_time_s` <= @current_ts")
                ...
                cluster_totals[cluster] = cluster_totals.get(cluster,
                                                             0) + template_total
            ...
            current_top_cluster_names = [cluster for cluster, _ in
                                         current_top_clusters]
            for template, cluster in assignment.items():
                ...
                query = clusterer._df.query(
                    "`query_template` == @template and @start_ts <= `log_time_s` and `log_time_s` < @end_ts")
                online_clusters[cluster] = online_clusters[cluster].add(query,
                                                                        fill_value=0)
